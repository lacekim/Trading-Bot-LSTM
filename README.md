...
The scheduler loads this model only when `models/smc_bearish_calibration.json` records `promoted: true`. Training uses four chronological, non-overlapping partitions: 70% training, 10% model validation, 10% threshold calibration, and a final untouched 10% promotion holdout. Promotion is per asset: thresholds are selected only on the calibration partition, while final-holdout precision must be at least 0.55 and all three non-overlapping holdout windows must have positive net return and profit factor of at least 1.30. The current artifact promotes no SHORT symbols because none passed the untouched holdout; the scheduler therefore executes no model-based SHORT until a future retraining genuinely passes promotion. Calibrated thresholds are stored with the model, while detailed results are written to `reports/v5_bearish_validation.csv` and `reports/v5_bearish_walk_forward.csv`.
