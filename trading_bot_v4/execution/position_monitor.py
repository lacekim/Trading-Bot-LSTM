"""Minute-level protective monitoring for persistent paper positions.

Entry/model decisions remain hourly. This service only marks open positions and
executes stored stop-loss or take-profit protection using GMX one-minute data.
"""
...
class GMXMinutePriceProvider:
    ...
    def fetch(self, symbol: str) -> MarketPrice:
...
class PositionMonitor:
    def __init__(self, controller: ShutdownController, notifier: AlertSink | None = None,
        ...
        self.provider = provider or GMXMinutePriceProvider()
        ...
        if errors:
            ...
            if self._consecutive_failures >= self.failure_threshold and not self._failure_alerted:
                ...
                self._alert(
                    ...
                    f"Price feed failed {self._consecutive_failures} consecutive times.\n"
